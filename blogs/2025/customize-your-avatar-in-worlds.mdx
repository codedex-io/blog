...
Building a customization system sounds simple: 1️⃣ add the outfit/hair style, 2️⃣ pick a color, 3️⃣ apply it to the sprite and boom! Done, right?

Well, not quite. Here’s what I had to consider:

### Static Assets Combination

7 skin tones, 3 hair styles with 10 hair colors, 4 outfits with 5 different colors for each outfit and 8 background colors. The number of combinations is:

$$
7 \times 3 \times 10 \times 4 \times 5 \times 8 = 33{,}600
$$

Creating a separate sprite file for each? That’s over **33,600 asset files for \*one\* character**, across walking and standing animations in 4 directions. NO THANKS! 

### Preserving pixel-art depth

You can't just find-and-replace colors. A sprite has shadows, mid-tones, and highlights. Swap "brown" for "pink" and you lose all that shading. The character looks flat and lifeless.

### Real-time preview

The Worlds runs in [Phaser](https://codedex.io/phaser), but the customizer lives in [React](https://codedex.io/react) outside of the Worlds. I needed instant visual feedback without booting up a game engine every time you tweak a style for your character.

### Layer synchronization

Body, hair, and outfit are separate sprite layers. They all need to animate together, frame-perfectly, in all four directions. Imagine a game world where your character walks and the hair comes floating behind it. Yeah, I wouldn’t play that game either!

So what did I do to address these issues, you ask?
...
It has 10 frames in each row, with 4 rows. A total of 40 frames.
...
Shoutout to our brilliant pixel artist **Azuna Pixels** for handcrafting these assets and our in-house Sr. Product Designer **Jackie Liu** for the color palettes and UI/UX designs!
...
**Note**: "Compression tolerance" refers to small color shifts that happen when images are saved or compressed. A pixel that should be exactly #341300 might become #351401.

The `tolerance = 15` means: if a pixel is close enough to the target color (within 15 units), we still consider it a match and swap it.

This would often happen in production environment because of all the optimizations enabled in production such as asset compression.
...
Hundreds of unique combinations from ~20 base sprite files. No 10,000-asset nightmare. Just math, pixels, and a lot of for-loops running twice – once for your preview, once in the game.
...
Worlds is where our community comes together – for events, hackathons (raffles and winner announcements), and just hanging out. But when everyone looks the same, it's _stale_.

Now you can stand out. And spot that bright pink hair across the room instantly.
