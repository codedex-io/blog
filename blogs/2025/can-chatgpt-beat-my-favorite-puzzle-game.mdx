...
So, I decided to find out which [SOTA model](https://www.google.com/search?q=SOTA+model) is the "smartest" by creating my own [Clues By Sam LLM benchmarking tool](https://github.com/Syps/cbs-bench) using Python and langchain. After comparing performance across 25 puzzles of varying difficulties...
...
<div style={{ display: "flex", justifyContent: "center", marginBottom: "1.2rem", transform: "scale(.9)" }}>
  <a href="https://cluesbysam.com/" style={{ all: "unset" }} target="_blank" and rel="noopener noreferrer"><Button variant="yellow">Try Clues By Sam</Button></a>
</div>

<Quote text="At that point, how do we know whether the models are actually solving the puzzles or just memorizing nuanced patterns of the ones they see in training.” - Nick Sypteras" />
...
All puzzles in the evaluation came from Clues By Sam's [Puzzle Pack #1](https://cluesbysam.com/s/puzzle-pack-1.html), a pay-what-you-want bonus puzzle pack. There are five difficulty levels:
...
- **💸 Most pricey** - Gemini 2.5 Pro, high cost and high error rate (hence more re-prompting)
...
- **🤪 Dumbest** - Grok 4, not being able solve even the easiest puzzle. I want my $10 back.
...
You can try comparing the models yourself by [cloning the repository](https://github.com/Syps/cbs-bench). 

<div style={{ display: "flex", justifyContent: "center", marginBottom: "1.2rem", transform: "scale(.9)" }}>
  <a href="https://github.com/Syps/cbs-bench" style={{ all: "unset" }} target="_blank" and rel="noopener noreferrer"><Button variant="yellow">View GitHub Repo</Button></a>
</div>

There's even a "race mode" to run the models in parallel against a given puzzle.
...
Rather than using browser automation, the tool creates a serialized version of Clues By Sam using ASCII text that can be easily tokenized and passed to the LLM. 
...
The serialized game state consists of an ASCII grid followed by hints for each revealed person.
...
The evaluation code is essentially a single agentic loop. 

An **agentic loop** is an iterative process where an AI takes a goal, performs an action, evaluates the result, and then repeats the process to improve its outcome or complete a complex task.

The prompt starts with the game rules followed by the puzzle state. The model responds with a summary of its "logic" and a move which follows the pattern: 

```output
MOVE: <name> is <criminal/innocent>
```
...
Quite verbose for how simple the hint is, but a correct move nonetheless. 

Interestingly, the model's thought summary immediately begins with a hallucination. 

A **hallucination** is when a model generates false/inaccurate info while presenting it as factual.

It initially misidentifies the common neighbors of Gary and Isaac only to correctly identify them further down in its output.

## Validating The Output and Re-Prompting

The `MOVE` then gets extracted from the response via [regex](https://en.wikipedia.org/wiki/Regular_expression) and validated. 

How do we know if the move is correct or not? Thankfully, the answers are scrapable from the webpage that hosts the puzzle. The benchmark tool I wrote has logic to fetch the raw source and extract the answers into a nice object we can validate against. 

You can see that logic [here](https://github.com/Syps/cbs-bench/blob/main/fetch.py#L15).

The code updates the model context with the result (incorrect/correct), the incorrect reason if applicable (wrong determination or not logically deducible), and the updated state of the puzzle. 

To help the model not repeat its mistakes, incorrect moves are kept "in memory" by passing them in all subsequent prompts. With the context updated, we send a new completion request to the LLM provider via [langchain](https://docs.langchain.com/oss/python/langchain/models).

The model responds with a next move, and the process repeats. The evaluation ends when either the puzzle has been fully solved or the model has made over 40 moves. 

Why 40? Since the the puzzle is always a 4x5 grid with one cell revealed by default, each game only requires 19 moves assuming no mistakes. So I chose 40 moves as an abritrary "safety brake" to prevent cost overrun.
...
## So why is GPT-5 so good at this?

GPT-5 far surpassed my expectations of how well a SOTA LLM could perform. 

And by that I mean... it beats me. A lot. 😭

GPT-5's knowledge cutoff is [September 2024](https://platform.openai.com/docs/models/compare). Based on the creator's [original HN post](https://news.ycombinator.com/item?id=43821457), Clues By Sam went live in April 2025. We can therefore assume that no information about the game exists in GPT-5's training data. 

So how does it perform so well? Is it actually capable of "reasoning"?

Researchers have found that [the success of LLMs at logical problems depends more on recognizing patterns from the training set](https://arxiv.org/abs/2406.11050) rather than actional reasoning ability. 

A good example of this phenomenom can be seen in LLM performance on [math benchmarks](https://en.wikipedia.org/wiki/Language_model_benchmark#Mathematics). Simply change some numbers or the phrasing of the math problems, and [model performance takes a significant hit](https://arxiv.org/abs/2410.05229).

So maybe in the case of Clues By Sam, either:
...
So it does sound like there are a decent number of similar games in the training data. 

Still, thinking about whether this is *pattern matching* or *reasoning* makes my brain hurt. Are they the same thing at some level?
...
In the all-you-can-crawl buffet that is AI company data ingestion, Clues By Sam, maybe even this blog post, will inevitably be trained on. 

At that point, how do we know whether the models are actually solving the puzzles or just memorizing nuanced patterns of the ones they see in training (let's for now assume this is conceptually different than "reasoning")?

One potential solution I can imagine would be an adversarial approach in which two LLMs are pitted against each other:

- LLM A creates new logic puzzles and 
- LLM B attempts to solve them. 

Could this potentially assess LLM A's "creativity" (it's ability to create logically satisfiable and novel puzzles) while measuring LLM B's "reasoning" ability?
...
- [DeekSeek Chat](https://chat.deepseek.com)
