...
An **agentic loop** is an iterative process where an AI agent takes a goal, performs an action, evaluates the result, and then repeats the process to improve its outcome or complete a complex task.

The prompt starts with the game rules followed by the puzzle state. The model responds with a summary of its "logic" and a move which follows the pattern `MOVE: <name> is <criminal/innocent>`.
...
An **hallucination** is when a model generates false, inaccurate, or nonsensical information while presenting it as factual.
