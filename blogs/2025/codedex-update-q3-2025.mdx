...
Immediately after the v1.0 launch, we shipped out the global search. Over the last few months, we've getting requests on how to find friends on Codédex. So we decided to add it to global header so it's easily accesible on every page outside of the learning enviornment.
